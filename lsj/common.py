...
def file_load(wav_name, mono=False):
    ...
    """
    audio = tf.io.read_file(wav_name)
    return tf.audio.decode_wav(contents=audio)
...
def file_to_vectors(file_name,
    ...
    #                                                  power=power)
    y = tf.squeeze(y, -1)
    y = tf.squeeze(tf.pad(tf.expand_dims(y, 0), [[0,0],[n_fft//2, n_fft//2]]))
    spectrogram = tfio.audio.spectrogram(y, n_fft, n_fft, hop_length) ** power
    mel_spectrogram = tf.transpose(tfio.audio.melscale(spectrogram, sr, n_mels, fmin=0, fmax=tf.cast(sr//2, tf.float32)), (1,0))
    # convert melspectrogram to log mel energies
    log_mel_spectrogram = 20.0 / power * tf.math.log(tf.math.maximum(mel_spectrogram, sys.float_info.epsilon)) / tf.math.log(tf.ones_like(mel_spectrogram) * 10)
    ...
    log_mel_spectrogram = tf.transpose(log_mel_spectrogram, (1,0))

    vectors = tf.signal.frame(log_mel_spectrogram, n_vectors, 1, axis=0)
    vectors = tf.reshape(tf.transpose(vectors, [1,0,2]), (n_vectors, -1))
